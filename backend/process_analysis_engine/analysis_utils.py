...
def get_activity_duration(event_log: pd.DataFrame) -> pd.DataFrame:
    """
    Calculates overall and mean activity durations (in hours) from an event log DataFrame.
    Supports logs with or without lifecycle transitions by applying appropriate duration logic.
    """
    if "lifecycle:transition" in event_log.columns:
        ...
        .apply(lambda x: (((x.shift(-1)-x).dt.total_seconds())/3600).round(2))
        .reset_index())
...
def get_rework_stats(event_log: pd.DataFrame) -> dict:
    """
    Computes the number of rework cases per activity from an event log.
    Ensures that activities without rework are included with a count of zero.
    """
